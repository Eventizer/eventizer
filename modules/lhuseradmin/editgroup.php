...
$tpl = erLhcoreClassTemplate::getInstance('lhuseradmin/editgroup.tpl.php');
...
if (isset($_POST['assignUsersAction']) ) {
	...
	if (isset($_POST['UserID']) && count($_POST['UserID']) > 0) {
		...
		$tpl->set('errorsAssignUsers',array(__t('system/message','Select users to assign')));
	}
...
if (isset($_POST['removeUserFromGroupAction'])) {
	...
	if(isset($_POST['AssignedID']) && count($_POST['AssignedID']) > 0) {
		...
		$tpl->set('errorsAssignUsers',array( __t('system/message','Select assigned users')));
	}
...
if (isset($_POST['assignRolesAction']) ) {
	...
	if(isset($_POST['RoleID']) && count($_POST['RoleID']) > 0) {
		...
		$tpl->set('errorsAssignRoles',array(__t('system/message','Select roles to assign')));
	}
...
if (isset($_POST['removeRoleFomGroupAction']) ) {
	...
	if(isset($_POST['AssignedID']) && count($_POST['AssignedID']) > 0) {
		...
		$tpl->set('errorsAssignRoles',array( __t('system/message','Select assigned roles')));		
	}
	
}

$pages = new lhPaginator();
$pages->items_total = erLhcoreClassModelGroupUser::getCount(array('filter' => array('group_id' => $groupData->id)));
$pages->setItemsPerPage(20);
$pages->serverURL = erLhcoreClassDesign::baseurl('useradmin/editgroup').'/'.$groupData->id;
$pages->paginate();

$tpl->set('pages',$pages);

if ($pages->items_total > 0) {
    $tpl->set('assignedUers',erLhcoreClassModelGroupUser::getList(array('filter' => array('group_id' => $groupData->id),'offset' => $pages->low, 'limit' => $pages->items_per_page )));
} else {
...
$Result['title'] = __t('useradmin/editgroups','Group edit');
$Result['small_title'] = $groupData->name;
...
$Result['path'] = array(
	array('url' => __url('useradmin/grouplist'),'title' => __t('user/editgroup','Groups')),
	array('title' => __t('user/editgroup','Group edit').' - '.$groupData->name)
)
