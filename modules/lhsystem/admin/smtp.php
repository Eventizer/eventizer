...
if ( isset($_POST['StoreSMTPSettings']) || isset($_POST['StoreSMTPSettingsTest']) ) {
	...
	$smtpData->saveThis();
	$tpl->set('alertSuccessAction',__t('system/smtp','SMTP data updated'));

	if (isset($_POST['StoreSMTPSettingsTest'])){
		try {
			erLhcoreClassMail::sendTestMail($currentUser->getUserData());
			$tpl->set('alertSuccessAction',__t('system/smtp','SMTP test successful'));
		} catch (Exception $e) {
