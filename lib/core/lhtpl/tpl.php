...
class erLhcoreClassTemplate {
	...
	function fetch($fileTemplate = null) {
		...
		if ($this->templatecompile == true) {
			...
			// Global functions
			$Matches = array ();
			preg_match_all ( '/<\?=__t\(\'(.*?)\',\'(.*?)\'\)(.*?)\?\>/i', $contentFile, $Matches );
			foreach ( $Matches [1] as $key => $TranslateContent ) {
				$contentFile = str_replace ( $Matches [0] [$key], __t ( $TranslateContent, $Matches [2] [$key] ), $contentFile );
			}
			
			// Global functions
			$Matches = array ();
			preg_match_all ( '/<\?php echo __t\(\'(.*?)\',\'(.*?)\'\)(.*?)\?\>/i', $contentFile, $Matches );
			foreach ( $Matches [1] as $key => $TranslateContent ) {
				$contentFile = str_replace ( $Matches [0] [$key], __t ( $TranslateContent, $Matches [2] [$key] ), $contentFile );
			}
			...
			$Matches = array ();
			preg_match_all ( '/<\?=__url\((.*?)\)(.*?)\?\>/i', $contentFile, $Matches );
			foreach ( $Matches [1] as $key => $UrlAddress ) {
				$contentFile = str_replace ( $Matches [0] [$key], __url ( trim ( $UrlAddress, '\'' ) ), $contentFile );
			}
			
			$Matches = array ();
			preg_match_all ( '/<\?=erLhcoreClassModelArticleCategory::fetch\((.*?)\)->url_path(.*?)\?\>/i', $contentFile, $Matches );
			foreach ( $Matches [1] as $key => $UrlAddress ) {
				$contentFile = str_replace ( $Matches [0] [$key], erLhcoreClassModelArticleCategory::fetch ( trim ( $UrlAddress, '\'' ) )->url_path, $contentFile );
			}
			
			$Matches = array ();
			preg_match_all ( '/<\?php echo erLhcoreClassModelArticleStatic::fetch\((.*?)\)->content_front(.*?)\?\>/i', $contentFile, $Matches );
			foreach ( $Matches [1] as $key => $UrlAddress ) {
				$contentFile = str_replace ( $Matches [0] [$key], erLhcoreClassModelArticleStatic::fetch ( trim ( $UrlAddress, '\'' ) )->content_front, $contentFile );
			}
			
			$Matches = array ();
			preg_match_all ( '/<\?=erLhcoreClassModelArticleCategory::fetch\((.*?)\)->category_name(.*?)\?\>/i', $contentFile, $Matches );
			foreach ( $Matches [1] as $key => $UrlAddress ) {
				$contentFile = str_replace ( $Matches [0] [$key], erLhcoreClassModelArticleCategory::fetch ( trim ( $UrlAddress, '\'' ) )->category_name, $contentFile );
			}
			...
			$Matches = array ();
			preg_match_all ( '/<\?php echo __url\((.*?)\)(.*?)\?\>/i', $contentFile, $Matches );
			foreach ( $Matches [1] as $key => $UrlAddress ) {
				$contentFile = str_replace ( $Matches [0] [$key], __url ( trim ( $UrlAddress, '\'' ) ), $contentFile );
			}
