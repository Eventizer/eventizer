...
class erLhcoreClassModule {
		...
		if (isset(self::$currentModule[self::$currentView])) {
			...
			try {
				...
				if (erConfigClassLhConfig::getInstance()->getSetting('site', 'debug_output') == true) {
					...
					$debugEmail = erConfigClassLhConfig::getInstance()->getSetting( 'site', 'debug_email' );					
					if($debugEmail != false) {
	...
	public static function getModuleDefaultView($module) {
		...
		if (isset($Module['default_function']))
			return $Module['default_function'];
	...
	private static $currentModuleName = NULL;

	private static $currentView = NULL;

	private static $moduleCacheEnabled = NULL;

	private static $cacheInstance = NULL;

	private static $cacheVersionSite = NULL;
