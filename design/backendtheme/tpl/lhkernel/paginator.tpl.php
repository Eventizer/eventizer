<?php if (isset($pages) && $pages->num_pages > 1) : ?>
	...
	<div class="row">	
		<div class="col-xs-6">
			<div class="dataTables_info"><?=__t('core/paginator','Page')?> <?=$pages->current_page?> <?=__t('core/paginator','of')?> <?=$pages->num_pages?>, <?=__t('core/paginator','Total')?>: <?=$pages->items_total?></div>
		</div>
		<div class="col-xs-6">
	    		...
	    		<?php if($pages->num_pages > 10): ?>
				    ...
				    <?php if($pages->range[0] > 1): ?>				    
				    	...
				    	<?php if($i == $pages->current_page) : ?>
				    	   <li class="active"><a title="<?=__t('core/paginator','Go to page')?> <?=$i?> <?=__t('core/paginator','of')?> <?=$pages->num_pages?>" href="#"><?=$i?></a></li>
				    	<?php else : ?>
